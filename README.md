...
- **Parser:** Snail uses [JuliaSyntax](https://github.com/JuliaLang/JuliaSyntax.jl) to infer the structure of source files and to enable features which require an understanding of code context, especially the module in which a particular piece of code lives. On Julia 1.10 and later Snail uses the built-in `Base.JuliaSyntax`; on older Julia versions it bootstraps `JuliaSyntax.jl` into a Snail-managed environment. This enables awareness of the current module for completion, cross-referencing, and nested `include` tracking.
...
Snail supports Julia 1.6 and later.

Core Snail has no always-on Julia-side dependencies. On Julia versions <1.10, or when using extensions that have Julia dependencies, Snail will bootstrap those dependencies on startup and keep them out of your way using Julia’s [`LOAD_PATH` mechanism](https://docs.julialang.org/en/v1/base/constants/#Base.LOAD_PATH).
